...
exports.createCourse = async (req, res) => {
  try {
    ...
    // Verify instructor and category
    const instructorDetails = await User.findById(userId, { accountType: "Instructor" });
    if (!instructorDetails) return res.status(404).json({ success: false, message: "Instructor Details Not Found" });

    const categoryDetails = await Category.findById(category);
    if (!categoryDetails) return res.status(404).json({ success: false, message: "Category Details Not Found" });
